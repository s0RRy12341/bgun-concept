      ...
      >
            ...
            state.items.map((item) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                    ...
                    <div className="mt-1 space-y-0.5">
                      {Object.entries(item.selectedVariants).map(([key, val]) => (
                        <p key={key} className="text-zinc-500 text-xs">
                          {key}: <span className="text-zinc-600">{val}</span>
                        </p>
                      ))}
                    </div>
                  )}
                  ...
                  <div className="flex items-center justify-between mt-2">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <span className="text-black text-sm w-6 text-center font-medium">
                        {item.quantity}
                      </span>
                      <button
                        onClick={() => updateQuantity(item.product.id, item.quantity + 1)}
                ...
                <button
                  ...
                  className="text-zinc-400 hover:text-red-500 transition-colors self-start mt-1 flex-shrink-0"
                  aria-label="הסר מוצר"
                >
        ...
        {state.items.length > 0 && (
          <div className="p-5 border-t border-[#e0d9c8] space-y-4">
            ...
            <button
              onClick={closeCart}
              className="w-full text-center text-zinc-500 hover:text-black text-sm transition-colors py-1"
            >
              המשך בקנייה
            </button>
          </div>
